#!/usr/bin/env bash
# VPS 自动初始化部署脚本 v2（Debian/Ubuntu）
# 请以 root 身份执行：bash vps_init.sh
...
download_https() {
    local url="$1" destination="$2" temporary="$destination.tmp.$$"
...
NET_TCP_TUNE_URL="${NET_TCP_TUNE_URL:-https://raw.githubusercontent.com/AthesFrey/VPS_sh/main/net-tcp-tunev2.sh}"
download_https "$NET_TCP_TUNE_URL" /root/net-tcp-tunev2.sh
bash /root/net-tcp-tunev2.sh

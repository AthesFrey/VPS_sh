...
nft_escape_string() {
    # Escape text for nft double-quoted strings, such as log prefix values.
    # This prevents values like "nft-new: " from being emitted without quotes.
...
# Generated by nft_firewall_manager_own_table_only_v6.sh
...
EOF_NFT
    ...
    if [ -n "$tcp_ports" ]; then
        printf '        ct state new tcp dport { %s } log prefix "%s" level info comment "nftfw-manager: log managed tcp"\n' "$tcp_set" "$log_prefix_escaped" >> "$out"
        printf '        tcp dport { %s } accept comment "nftfw-manager: accept managed tcp"\n' "$tcp_set" >> "$out"
    fi

    if [ -n "$udp_ports" ]; then
        printf '        ct state new udp dport { %s } log prefix "%s" level info comment "nftfw-manager: log managed udp"\n' "$udp_set" "$log_prefix_escaped" >> "$out"
        printf '        udp dport { %s } accept comment "nftfw-manager: accept managed udp"\n' "$udp_set" >> "$out"
    fi
...
EOF_NFT
        printf '        ct state new log prefix "%s" level info comment "nftfw-manager: log unmanaged new"\n' "$log_prefix_escaped" >> "$out"
        cat >> "$out" <<EOF_NFT
...
show_help() {
    cat <<EOF_HELP
Usage:
  bash /root/nft_firewall_manager_own_table_only_v6.sh
...
Modes:
  ...
  Strict mode, optional and dangerous if your saved lists are incomplete:
    STRICT_DROP=1 bash /root/nft_firewall_manager_own_table_only_v6.sh
...
Examples:
  JOURNAL_LIMIT=100M bash /root/nft_firewall_manager_own_table_only_v6.sh
  LOG_PREFIX_NFT='nft-new: ' bash /root/nft_firewall_manager_own_table_only_v6.sh
EOF_HELP
}
...
menu() {
    while true; do
        echo ""
        echo "===== NFT FIREWALL MANAGER OWN TABLE ONLY v6 ====="
        show_nftables_active_status

...
# 说明：我们仍然记录“新连接”，并对日志做限速；22/80/443 默认不记
...
table inet connlog {
  chain input {
    ...
    # 只记录新连接（不改变判决），加速率限制，避免刷爆日志
    ct state new counter limit rate 10/second burst 50 packets log prefix "conn " flags all

    # 如需减少 UDP 噪声，可启用端口排除（默认注释）
    # meta l4proto udp udp dport { ${CONN_UDP_EXCLUDE} } return
  }
}
EOF

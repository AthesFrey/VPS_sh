...
# nft_firewall_manager_unified_v9.sh
...
#
# Main design:
# - Normal Apply writes a complete /etc/nftables.conf and reloads nftables.
# - The generated config uses `flush ruleset`, so active legacy/iptables-nft tables
#   are removed when Apply/Initialize is run.
...
# Apply configures the real main firewall by default.
# It overwrites NFT_CONF and restarts/applies nftables.
# Because the generated config uses `flush ruleset`, it removes foreign tables.
# Set SKIP_FOREIGN_TABLE_CONFIRM=1 only if you know this is expected.
...
show_foreign_tables_warning() {
    ...
    if [ -n "$foreign" ]; then
        ...
        printf '%s\n' "$foreign"
        echo "[WARN] This unified script writes a config with 'flush ruleset'."
        echo "[WARN] Apply/Initialize will remove those active foreign tables, including iptables-nft tables."
        return 0
    fi
    return 1
}
...
confirm_if_foreign_tables_exist() {
    ...
    if show_foreign_tables_warning; then
        echo "[WARN] Continue only if you want one clean nftables table/config."
...
# Generated by nft_firewall_manager_unified_v9.sh
# Unified target: one persistent config and one nft table.
# Config file: $NFT_CONF
# Table: $MGR_FAMILY $MGR_TABLE
# WARNING: this file intentionally flushes the full ruleset before rebuilding
# the unified table. This removes foreign/iptables-nft generated tables.
...
apply_config_file() {
    ...
    echo "[+] Writing unified nftables config: $NFT_CONF"
    cp "$tmp" "$NFT_CONF"
    ...
    if has_cmd systemctl; then
        echo "[+] Restarting nftables via systemctl..."
        ...
        echo "[ERROR] systemctl restart nftables failed. Attempting rollback..."
    ...
    echo "[+] systemctl not found; applying directly with nft -f..."
    if nft -f "$NFT_CONF"; then
        echo "[OK] $action_name completed by direct nft load."
        rm -f "$backup_conf"
        return 0
    fi

    echo "[ERROR] Direct nft apply failed. Attempting rollback..."
    if [ "$had_old_conf" -eq 1 ]; then
        cp "$backup_conf" "$NFT_CONF"
        nft -f "$NFT_CONF" || true
    fi
...
apply_changes() {
    tmp="$(mktemp)"
    build_unified_config "$tmp"

    echo "[+] Applying unified main nftables table: $MGR_FAMILY $MGR_TABLE"
    if apply_config_file "$tmp" "Apply"; then
        rm -f "$tmp"
        echo "[OK] Managed TCP ports: $(csv_from_file "$TCP_FILE")"
        echo "[OK] Managed UDP ports: $(csv_from_file "$UDP_FILE")"
        echo "[OK] Note: this version intentionally manages the real main table, not a low-priority side table."
        return 0
    fi
...
initialize_nft_safe() {
    ...
    if apply_config_file "$tmp" "Emergency initialize"; then
        rm -f "$tmp"
...
add_ports() {
    ...
    read -r -p "Apply now? This rewrites $NFT_CONF and restarts nftables. (y/n): " yn
...
show_help() {
    cat <<EOF_HELP
Usage:
  bash /root/nft_firewall_manager_unified_v9.sh
...
Main behavior in this version:
  ...
  3. Generated config uses:
       flush ruleset
     Therefore Apply/Initialize removes active foreign tables, including tables
     created by iptables-nft/ip6tables-nft.
  4. Input chain default is drop, with explicit allow rules for saved TCP/UDP ports.
  5. Forward is drop. Output is accept.
  6. Emergency Initialize resets to TCP 22,80,443 and empty UDP.
...
Why this is different from the old own-table script:
  ...
  could fail to override drops in another base chain. This unified version writes
  the real main table, so its allow/drop decisions are authoritative after Apply.
...
Important caution:
  If your VPS depends on NAT/DNAT rules created by iptables-nft, Docker, or another
  tool, Apply/Initialize may remove those active tables because this script keeps
  one clean nftables ruleset. Backups are saved under:
    $BACKUP_DIR
EOF_HELP
}
...
menu() {
    while true; do
        echo ""
        echo "===== NFT FIREWALL MANAGER UNIFIED MAIN TABLE v9 ====="
        show_nftables_active_status
...
show_cli_usage() {
    cat <<EOF_USAGE
Usage:
  bash nft_firewall_manager_unified_v9.sh
  bash nft_firewall_manager_unified_v9.sh --apply
  bash nft_firewall_manager_unified_v9.sh --init-safe
  bash nft_firewall_manager_unified_v9.sh --show
  bash nft_firewall_manager_unified_v9.sh --preview
  bash nft_firewall_manager_unified_v9.sh --help

Options:
  --apply       Build $NFT_CONF from saved port lists and restart/apply nftables.
  ...
  --preview     Print the generated config without applying it.

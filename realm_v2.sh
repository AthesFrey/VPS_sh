#!/usr/bin/env bash
# Realm deployment script v2. Official stable releases, Debian/Ubuntu + systemd.
...
def main():
    ...
    if cmd == "validate":
        ...
        for i, ep in enumerate(items, 1):
            # Old scripts put remarks immediately before [[endpoints]]. Keep all
            # original comments; new remarks are attached to the listen key.
        ...
        net.update({"no_tcp": False, "use_udp": args[4] == "true"})
        ep.add("network", net)
        if not isinstance(doc.get("endpoints"), tomlkit.items.AoT):
            replacement = tomlkit.aot()
            for old in items:
                table = tomlkit.table()
                table.update(old)
                replacement.append(table)
            doc["endpoints"] = replacement
        ...
        if not doc["endpoints"]:
            del doc["endpoints"]
            # A normal array must be a root value, before all TOML tables.
            new_doc = tomlkit.document()
            new_doc.add("endpoints", [])
            text = tomlkit.dumps(new_doc) + "\n" + tomlkit.dumps(doc)
...
ensure_cmds() {
  ...
  if ((${#missing[@]})); then
    if [[ $EUID != 0 ]] || ! command -v apt-get >/dev/null 2>&1; then
      err "缺少依赖：${missing[*]}。Debian/Ubuntu 请由管理员执行：apt-get update && apt-get install -y ${packages[*]}"
      return 1
    fi
...
cleanup_workspace() {
  ...
  if [[ -n $TMP_DIR && ${TMP_DIR##*/} == realm-v2.* ]]; then
    rm -rf -- "$TMP_DIR"
  fi
  TMP_DIR=""
  EXEC_DIR=""
...
prepare_workspace() {
  ...
  TMP_DIR=$(mktemp -d "${REALM_TMPDIR:-${TMPDIR:-/tmp}}/realm-v2.XXXXXX") || return 1
...
fetch_binary() {
  ...
  log "官方最新稳定版：$LATEST_TAG；架构：$(uname -m)；优先使用 musl 兼容构建。"
...
make_default_config() {
  printf '%s\n' '# Realm configuration. Add endpoints through realm_v2.sh.' \
...
add_endpoint_interactive() {
  local listen remote remark udp use_udp candidate
  ...
  while true; do
    ...
    read -r -p '是否同时转发 UDP？(y/N): ' udp || return 1
    use_udp=false
    [[ ${udp,,} != y && ${udp,,} != yes ]] || use_udp=true
    read -r -p '备注（可留空）: ' remark || return 1
    helper add "$CONF_FILE" "$candidate" "$listen" "$remote" "$use_udp" "$remark" || return 1
    commit_config "$candidate" || return 1
    log "已添加节点：$listen -> $remote（UDP=$use_udp）"
...
usage() {
  cat <<EOF
Realm 部署脚本 v2（Debian/Ubuntu + systemd）
用法：bash $0 [选项]
  无参数                     打开交互菜单
...
缺少依赖时使用 apt-get 安装 curl、python3、python3-tomlkit 等发行版软件包。
...
main_menu() {
  ...
  while true; do
    printf '\n%s\n' '==== Realm 管理 v2 ====' \
      ...
      '3) 卸载' '4) 查看状态与节点' '5) 添加转发节点' '6) 删除 / 批量删除转发节点' 'q) 退出'
...
main() {
  ...
  [[ $EUID == 0 ]] || { err '请使用 root 运行：sudo bash realm_v2.sh'; return 1; }
  [[ -d /run/systemd/system ]] || { err '当前系统未由 systemd 管理。'; return 1; }
  ensure_cmds || return 1
  mkdir -p /run/lock || return 1
  exec 9>/run/lock/realm-v2.lock
  flock -n 9 || { err '另一个 realm_v2.sh 正在运行。'; return 1; }
  case "$mode" in
    menu) main_menu ;;
